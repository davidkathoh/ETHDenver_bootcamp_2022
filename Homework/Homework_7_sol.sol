...
contract VolcanoCoin is Ownable{
    ...
    function getPayment(address user)   external view returns(Payment[] memory) {
        return payments[user];
    }
    
    function recordPayment(address sender, address receiver, uint amount) internal{

        payments[sender].push(Payment({recipient:receiver,amount:amount}));
